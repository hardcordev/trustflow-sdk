...
- Global coverage floor is set in `jest.config.js` (statements/lines/functions 55%, branches 43%), just under the current baseline, so a PR that lowers coverage fails CI
